...
function VRMModel({ url, emotion = 'warm', onProgress, onLoaded }: VRMModelProps) {
    ...
    loader.load(
      ...
      (xhr) => {
        ...
        if (xhr.lengthComputable && xhr.total > 0) {
          const percent = Math.min(Math.round((xhr.loaded / xhr.total) * 100), 98);
          if (onProgress) onProgress(percent);
        } else {
          // Chunked progressive approximation
          if (onProgress) {
            onProgress(Math.min(30 + Math.round((xhr.loaded / (3.5 * 1024 * 1024)) * 60), 95));
          }
        }
      },
      (error) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-20 transition-opacity duration-700">
      <div className="relative flex flex-col items-center justify-center">
        ...
        <div className="flex flex-col items-center gap-2.5 bg-black/60 backdrop-blur-xl border border-white/[0.08] px-5 py-3 rounded-2xl shadow-2xl">
