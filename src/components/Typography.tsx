...
export const Heading1: React.FC<TypographyProps> = ({ children, className = '' }) => (
  <h1 className={`font-heading font-medium text-2xl sm:text-[32px] text-[var(--text-primary)] leading-tight ${className}`}>{children}</h1>
);

export const Heading2: React.FC<TypographyProps> = ({ children, className = '' }) => (
  <h2 className={`font-heading font-medium text-lg sm:text-[22px] text-[var(--text-primary)] leading-snug ${className}`}>{children}</h2>
);
