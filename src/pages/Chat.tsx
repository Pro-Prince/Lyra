...
export default function Chat() {
            ...
            if (trimmedLine.startsWith('data: ')) {
               ...
               if (data?.text || data?.content) {
                  ...
                  if (emotion === 'excited' && Math.random() > 0.95 && isFirstChunk) {
                    // @ts-ignore
                    if (window.playGesture) window.playGesture(Math.random() > 0.5 ? 'laugh' : 'nod');
  ...
  if (isAdultVerified !== true || isLoadingProfile) {
    return (
                  ...
                  {messages.map((msg) => (
                    msg.role === 'user' ? (
                      ...
                      <div key={msg.id} className="self-start max-w-[95%] sm:max-w-[88%] flex gap-2.5 items-start">
                        <img src="/images/Logo.png" alt="Lyra" className="w-7.5 h-7.5 rounded-[8px] border-[1.5px] border-[#ff8fc0]/60 shrink-0 object-cover mt-0.5" />
                        <div className="flex flex-col items-start min-w-0">
                          <div className="bg-[var(--bg-panel)] text-[var(--text-primary)]/90 rounded-[18px] rounded-tl-[4px] p-3 px-3.5 shadow-xs border border-[var(--text-primary)]/10 relative group">
                            <p className="text-[14px] leading-relaxed break-words font-body">
                              <span className="whitespace-pre-wrap">{formatCleanMessageContent(msg.content)}</span>
                              <span className="inline-flex items-center gap-2 float-right ml-3 mt-1.5 align-bottom select-none">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.preventDefault();
                                    speakTextChunk(formatCleanMessageContent(msg.content), false);
                                  }}
                                  title="Listen to message"
                                  className="text-[var(--accent-primary)] hover:opacity-100 opacity-70 p-0.5 rounded transition-opacity cursor-pointer inline-flex items-center gap-1 text-[11px]"
                                >
